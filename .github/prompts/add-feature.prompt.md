...
`#skill-git-workflow` スキル（`.github/skills/git-workflow/SKILL.md`）の **「4. 作業開始手順（ブランチ作成）」** に従って実行する。

1. 現在のタスクコンテキストを確立する:
  - 機能名: `[引数で与えられた機能名]`
  - 日付: `[現在の日付をYYYYMMDD形式で取得]`
  - ブランチ名: `feature/[日付]-[機能名]`（スペースはハイフンに変換）
...
3. 未コミット変更がある場合は、まずコミットまたはstashして状態をクリーンにする。

4. mainブランチに移動して最新化する:
  ```bash
  git checkout main
  git pull origin main
  ```

5. 作業ブランチを作成してチェックアウトする:
  ```bash
  git checkout -b feature/[日付]-[機能名]
  ```

**このステップが正常に完了したら、決して停止せず、ただちにステップ1に進むこと。**
...
4. ステアリングファイル作成後、ドキュメントとしてコミットする:
  ```bash
  git add .steering/
  git commit -m "docs(steering): [機能名]のステアリングファイルを作成"
  ```
...
1. `#skill-steering` スキル（`.github/skills/steering/SKILL.md`）の指示に従って**計画モード**で実行し、ステップ1で作成した3つのファイル (`requirements.md`, `design.md`, `tasklist.md`) の内容を生成する。
...
3. タスクの実行:
  ...
  - `#skill-steering` スキル（`.github/skills/steering/SKILL.md`）の指示に従って**実装モード**で使用する。
  - `#skill-development-guidelines` スキル（`.github/skills/development-guidelines/SKILL.md`）の指示に従ってコーディング規約を常に遵守する。

4. タスクリストの更新:
  - 実行したタスクが完了したら、`Edit`ツールを使用して`tasklist.md`を更新し、該当タスクを `[ ]` から `[x]` に変更する。

5. **フェーズ完了時のコミット**（各Phaseの全タスクが `[x]` になった時のみ実行）:
  - `#skill-git-workflow` スキル（`.github/skills/git-workflow/SKILL.md`）の **「3. コミットタイミング」** に従い、コミットメッセージを生成する。
  - 実装ファイルと`.steering/`を**別々に**コミットする:
    ```bash
    # 実装ファイルのみ（docs/は含めない）
    git add src/ tests/
    git commit -m "[type]([scope]): [Phase内容を要約した説明]"

    # tasklist.md更新を別コミット
    git add .steering/
    git commit -m "docs(steering): [機能名] Phase[N]の進捗を記録"
    ```
...
2. `agent-implementation-validator.prompt.md`プロンプトを使用して品質を検証する。
  - `# agentモード参照`: "implementation-validator"
...
1. `#skill-steering` スキル（`.github/skills/steering/SKILL.md`）の指示に従って**振り返りモード**で実行し、`tasklist.md`に申し送り事項を記載する。
  - 実装完了日
  - 計画と実績の差分
  - 学んだこと
  - 次回への改善提案
...
`#skill-git-workflow` スキル（`.github/skills/git-workflow/SKILL.md`）の **「5. PR作成手順」** に従って実行する。
...
2. リモートにプッシュする:
  ```bash
  git push -u origin [ステップ0で作成したブランチ名]
  ```

3. ステアリングドキュメントからPR本文を生成する:
  - `[ステアリングディレクトリパス]/requirements.md` → 概要・背景
  - `[ステアリングディレクトリパス]/design.md` → 実装方針
  - `[ステアリングディレクトリパス]/tasklist.md` → 変更内容の箇条書き

4. `gh` コマンドが使用可能か確認し、使用可能な場合はPRを作成する:
  ```bash
  gh pr create \
    --title "feat: [機能名]" \
    --body "[生成したPR本文]" \
